package frp.utn.tp.security.dev;

import frp.utn.tp.security.AppRoles;
import frp.utn.tp.security.domain.UserId;
...
public final class SampleUsers {
     ...
     * </p>
     */
    static DevUser ADMIN = DevUser.builder().preferredUsername(ADMIN_USERNAME).fullName("Admin")
            .userId(ADMIN_ID).password(SAMPLE_PASSWORD).email("admin@example.com").roles(AppRoles.ADMIN, AppRoles.USER)
            .build();
     ...
     * </p>
     */
    static final DevUser USER = DevUser.builder().preferredUsername(USER_USERNAME).fullName("Usuario")
            .userId(USER_ID).password(SAMPLE_PASSWORD).email("user@example.com").roles(AppRoles.USER).build();
